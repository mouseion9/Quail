...
descent, max-width, and leading (suggested spacing between 
lines) of the host-font."
  (defvar ascent (clim:text-style-ascent host-font pane))
  (defvar descent (clim:text-style-descent  host-font pane))
  (defvar width (clim:text-style-width  host-font pane))
  (defvar leading (clim:text-style-height host-font pane))
            (values ascent descent width leading))
            
(eval-when (:load-toplevel)
  (host-font-description (clim:make-text-style :fix :roman :large)))
